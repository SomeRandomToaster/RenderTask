...
void VulkanRenderer::InitWindow() {
    ...
    window = SDL_CreateWindow(APP_NAME, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                                          WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_SHOWN | SDL_WINDOW_VULKAN);
    if (!window)
    {
        throw std::runtime_error(std::string("Creating window: ") + SDL_GetError());
    }
}
...
void VulkanRenderer::PickPhysicalDevice() {
    ...
    //No discrete GPU. Trying to pick any
    for (const auto& device : devices) {
...
bool VulkanRenderer::IsDeviceSuitable(VkPhysicalDevice device, bool discreteOnly = false) {
    ...
    return indices.IsComplete() && extensionSupport && swapChainSupport && (!discreteOnly || deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU);
}
